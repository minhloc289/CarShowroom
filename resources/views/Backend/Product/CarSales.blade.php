    ...
    <div class="post d-flex flex-column-fluid" id="kt_post">
        <div id="kt_content_container" class="container-xxl">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="dropdown">
                    <button class="btn btn-primary dropdown-toggle" id="addCarButton" data-bs-toggle="dropdown" aria-expanded="false">
                        Thêm
                    </button>
                    <ul class="dropdown-menu" aria-labelledby="addCarButton">
                        <li><a class="dropdown-item" href="">Thêm 1 xe</a></li>
                        <li><a class="dropdown-item" href="">Thêm nhiều xe</a></li>
                    </ul>
                </div>
            </div>
            ...
            <div id="carsContainer" class="d-flex flex-column">
                @foreach ($cars as $index => $car)
                    <div class="car-item align-items-center justify-content-between border bg-white shadow-sm p-4 rounded-lg mb-3" data-index="{{ $index }}" style="width: 100%;">
                        <div class="d-flex align-items-center">
                            <img src="{{ $car->image_url }}" class="rounded-lg" alt="{{ $car->name }}" style="width: 120px; height: auto;">
                            <h2 class="text-xl font-bold ms-3 mb-0">{{ $car->brand }} - {{ $car->model }}</h2>
                            <h2 class="text-xl font-bold ms-3 mb-0">{{ $car->name }}</h2>
                            <h2 class="text-xl font-bold ms-3 mb-0">Price: ${{ number_format($car->sale->sale_price, 2) }}</h2>
                        ...
                        <div class="d-flex align-items-center">
                            <a href="{{ route('show.car.details', ['carId' => $car->car_id]) }}"class="btn" style="background-color: #0194e7; color: white; margin-right: 8px;">Xem chi tiết</a>
                            <a href="{{ route('show.car.edit', ['carId' => $car->car_id]) }}" class="btn btn-warning me-2">Sửa</a>
                            <a href="" class="btn" style="background-color: #de3333; color: white;">Xóa</a>
                        </div>
                    </div>
                @endforeach
            </div>
