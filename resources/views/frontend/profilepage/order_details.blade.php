		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
			...
			<div>
				...
				<div>
					...
					<p class="text-gray-600 py-2"><strong>Số điện thoại:</strong> {{ $order->account->phone ?? 'N/A' }}
					</p>
					<p class="text-gray-600 py-2"><strong>Email:</strong> {{ $order->account->email ?? 'N/A' }}</p>
					<p class="text-gray-600 py-2"> <strong>CMND/CCCD:</strong>
						{{ $order->account->identity_card ?? 'N/A' }}</p>
					<p class="text-gray-600 py-2"><strong>Showroom nhận xe:</strong>
						{{ $order->showroom_name ?? 'N/A' }}</p>
				</div>
			</div>
					...
					<div class="bg-gray-100 rounded-lg p-4 mb-6">
						...
						<div class="flex items-center">
							<img src="{{ $order->salesCar->carDetails->image_url ?? 'default-image.jpg' }}" alt="Car Image"
								class="w-40 h-auto rounded-md shadow-md mr-4">
							<div>
								...
								<p><strong>Giá:</strong> {{ number_format($order->salesCar->sale_price, 0, ',', '.') }} VNĐ
								</p>
							</div>
						</div>
					</div>
						...
						@foreach ($order->accessories as $accessory)
							<div class="flex items-center mb-4">
								<img src="{{ $accessory->image_url ?? 'default-image.jpg' }}" alt="Accessory Image"
									class="w-16 h-auto rounded-md shadow-md mr-4">
								<div>
